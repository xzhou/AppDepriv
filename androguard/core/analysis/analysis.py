...
def get_dx_permissions(dx):
    """
    Show where the permission is used.
    :return:
    """
    p = dx.get_permissions([])
    ...
    for i in p:
        ...
        permissions_with_path[i] = paths
    return p, permissions_with_path
...
def get_path(vm, path):
    ...
    if isinstance(path, PathVar):
        dst_class_name, dst_method_name, dst_descriptor = path.get_dst(cm)
        info_var = path.get_var_info()
        result = "%s %s (0x%x) ---> %s->%s%s" % (path.get_access_flag(),
                                                 info_var, path.get_idx(),
                                                 dst_class_name,
                                                 dst_method_name,
                                                 dst_descriptor)
    else:
